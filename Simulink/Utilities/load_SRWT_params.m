function [R,F] = load_SRWT_params(P,simu)
%% Load SRWT Controller Parameters
...
R.Rgn2_Mode           = GetFASTPar(P.SD_dllP,'Rgn2_Mode'); % ADDED_SRWT % Region 2 control method
R.Rgn3_Mode           = GetFASTPar(P.SD_dllP,'Rgn3_Mode'); % ADDED_SRWT % Region 3 control method
R.M_Rgn3Lin           = GetFASTPar(P.SD_dllP,'M_Rgn3Lin'); % ADDED_SRWT % Angular coefficient, Region 3 linear method control mode
...
R.VS_TSRopt     = GetFASTPar(P.SD_dllP,'VS_TSRopt');  % Optimum TSR
...
R.VS_MinTq      = GetFASTPar(P.SD_dllP,'VS_MinTq');      % min torque
R.VS_RtTq       = GetFASTPar(P.SD_dllP,'VS_RtTq');      % rated torque
R.VS_RtPwr      = GetFASTPar(P.SD_dllP,'VS_RtPwr');     % rated power
R.VS_MaxRat     = GetFASTPar(P.SD_dllP,'VS_MaxRat');    % max torque rate

R.VS_RtWSpd     = GetFASTPar(P.SD_dllP,'VS_RtWSpd');    % ADDED_SRWT % wind speed at rated power 
...
R.WE_RhoAir         = GetFASTPar(P.ADP,'AirDens');  % ADDED_SRWT % Modified from: R.WE_RhoAir = GetFASTPar(P.SD_dllP,'WE_RhoAir')
